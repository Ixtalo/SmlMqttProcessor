...
def test_get_config_valid_file(mock_config_file, caplog):
    # action
...
def test_get_config_relative_path(mock_config_file, monkeypatch, caplog):
    # prepare
...
def test_get_config_file_not_found(tmp_path):
    ...
    # action
    with pytest.raises(FileNotFoundError, match=rf"No configfile! \({non_existent_file.resolve()}\)") as ex:
        get_config(non_existent_file)


def test_get_config_file_not_readable(non_readable_file):
    # action
    with pytest.raises(RuntimeError, match=rf"Configfile not readable! \({non_readable_file.resolve()}\)"):
